...
def generate(original, baseline_sha, candidate_sha, variant, campaign, sample, scope):
    ...
    if scope == 'full' and variant == 'candidate':
        matrix = '          INTEGRATION_TEST_PACKAGES=$(.github/scripts/affected-packages.sh test:integration)\n'
        annotation = '''
          GRAPH_BUILD_CACHE_SCOPES='["@rust/hash-graph-http-tests","@tests/hash-backend-integration","@tests/hash-playwright"]'
          INTEGRATION_TEST_PACKAGES=$(jq -c --argjson scopes "$GRAPH_BUILD_CACHE_SCOPES" '
            .name as $selected
            | ($scopes | map(select(. as $candidate | $selected | index($candidate))) | first // "") as $producer
            | .include |= map(.name as $name | . + {
                graph_build_cache: (($scopes | index($name)) != null),
                graph_build_cache_producer: ($name == $producer)
              })
          ' <<< "$INTEGRATION_TEST_PACKAGES")
'''
        jobs = replace_once(jobs, matrix, matrix + annotation)
    ...
    graph_mode = ("${{ matrix.graph_build_cache && '1' || '0' }}"
                  if variant == 'candidate' else '0')
    graph_mode = graph_mode if variant == 'candidate' else f'"{graph_mode}"'
...
'''
        ...
        include:
          - name: "@tests/hash-playwright"
            path: tests/hash-playwright
            graph_build_cache: true
            graph_build_cache_producer: false
          - name: "@tests/hash-backend-integration"
            path: tests/hash-backend-integration
            graph_build_cache: true
            graph_build_cache_producer: true
'''
        integration = replace_once(
            integration, '      matrix: ${{ fromJSON(needs.setup.outputs.integration-tests) }}\n', smoke_matrix)
...
'''
    ...
    publish_gate = ("github.event_name == 'push' && "
                    "(github.ref == 'refs/heads/main' || env.BENCHMARK_FORCE_GRAPH_CACHE_PUBLISH == '1') && "
                    "matrix.graph_build_cache_producer && steps.graph-build.outputs.key != '' && "
                    "steps.graph-cache.outputs.cache-hit != 'true'")
    validate_if = publish_gate if variant == 'candidate' else "${{ false }}"
    save_if = publish_gate + " && steps.graph-build-output.outputs.valid == 'true'"
    if variant == 'baseline':
        save_if = '${{ false }}'
    save = f'''      - name: Prepare graph build for upload
        id: graph-build-output
        if: {validate_if}
        env:
          EXPECTED_GRAPH_BUILD_KEY: ${{{{ steps.graph-build.outputs.key }}}}
        run: python3 .github/actions/graph-build-cache/graph_build_cache.py publish
...
'''
    ...
    receipt = f'''      - name: Record graph cache evidence
        ...
        env:
          ...
          GRAPH_CACHE_ENABLED: ${{{{ matrix.graph_build_cache }}}}
          GRAPH_CACHE_PRODUCER: ${{{{ matrix.graph_build_cache_producer }}}}
          GRAPH_MATRIX_NAME: ${{{{ matrix.name }}}}
          GRAPH_ACTIONS_KEY: "{key}"
          GRAPH_UPLOAD_VALID: ${{{{ steps.graph-build-output.outputs.valid }}}}
          GRAPH_SAVE_OUTCOME: ${{{{ steps.graph-cache-save.outcome }}}}
        run: |
          python3 - <<'PYTHON'
          ...
          receipt = {{key: os.environ.get(key, '') for key in [
              'GRAPH_MATRIX_NAME', 'GRAPH_CACHE_ENABLED', 'GRAPH_CACHE_PRODUCER',
              'GRAPH_PREPARED_KEY', 'GRAPH_ACTIONS_KEY', 'GRAPH_CACHE_HIT',
              'GRAPH_UPLOAD_VALID', 'GRAPH_SAVE_OUTCOME']}}
...
# Full scope reproduces the complete Test matrix; smoke scope runs two integrations.
...
name: Integration build benchmark ({variant} {sample}, {scope})
...
env:
  ...
  BENCHMARK_FORCE_GRAPH_CACHE_PUBLISH: "{'1' if variant == 'candidate' else '0'}"
    ...
    if args.self_test:
        for scope in ['smoke', 'full']:
            for variant in ['baseline', 'candidate']:
                ...
                assert generated.count('run: python3 .github/actions/graph-build-cache/graph_build_cache.py publish\n') == 1
                assert 'Recheck graph build cache' not in generated
                assert 'GRAPH_RECHECK_HIT' not in generated
                assert generated.count("matrix.graph_build_cache && '1' || '0'") == (1 if variant == 'candidate' else 0)
                assert generated.count("@tests/hash-backend-integration") == (1 if scope == 'smoke' or variant == 'candidate' else 0)
                assert generated.count("@rust/hash-graph-http-tests") == (1 if scope == 'full' and variant == 'candidate' else 0)
                ...
                assert ('  passed:\n' in generated) == (scope == 'full')
                assert generated.count('      - name: Check graph build reuse\n') == (1 if scope == 'full' and variant == 'candidate' else 0)
                assert ('  graph-build-cache-check:\n' in generated) == (scope == 'full' and variant == 'candidate')
                force = 1 if variant == 'candidate' else 0
                assert f'  BENCHMARK_FORCE_GRAPH_CACHE_PUBLISH: "{force}"\n' in generated
