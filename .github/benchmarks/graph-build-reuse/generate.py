...
'''
    ...
    integration = replace_once(integration, anchor, record + prepare + anchor)
    save = f'''      - name: Validate graph build for upload
        id: graph-upload
        if: env.HASH_GRAPH_BUILD_CACHE == '1' && steps.graph-build.outputs.key != '' && steps.graph-cache.outputs.cache-hit != 'true'
        env:
      ...
      - name: Save graph build
        if: steps.graph-upload.outputs.valid == 'true'
...
'''
    ...
    receipt = '''      - name: Record graph cache evidence
        if: always()
        env:
        ...
        run: |
          ...
          receipt = {key: os.environ.get(key, '') for key in ['GRAPH_PREPARED_KEY', 'GRAPH_CACHE_HIT', 'GRAPH_UPLOAD_VALID']}
    ...
    steps:
      ...
      - name: Check CPU cache-line decoding
        run: |
          ...
          "$CHECK_DIR/check"
      - name: Check timing comparison
        run: python3 .github/scripts/integration_build_timings_test.py
