...
fn arm_l1_data(ctr_el0: u64) -> u32 {
    4 << ((ctr_el0 >> 16) & 0xf)
}
...
fn native() -> Detection {
    let register: u64;
    unsafe { std::arch::asm!("mrs {value}, ctr_el0", value = out(reg) register) };
    Ok(arm_l1_data(register))
}
...
mod tests {
    ...
    #[test]
    fn arm_uses_only_dminline_and_kiddo_threshold() {
        assert_eq!(arm_l1_data(0xf), 4);
        assert_eq!(arm_l1_data((4 << 16) | 0xf), 64);
        assert_eq!(arm_l1_data(5 << 16), 128);
        assert_eq!(arm_l1_data(u64::MAX), 131_072);
    }
