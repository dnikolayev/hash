...
On x86, the probe requires usable `x86-64-v3` features on every permitted CPU,
...
ARM CPU identity fields remain inputs; missing ARM identity data bypasses reuse.
The native cache-line value used by `kiddo` is also an input. A small standalone
Rust probe reads the relevant CPUID fields on supported Intel/AMD hosts or
`CTR_EL0` on Linux ARM. Every permitted CPU must report the same usable value;
...
The optional `graph_cache_receipts` field maps exact job names to cache receipts.
Warm samples require an Actions cache hit and a matching-key helper restore
accepted before compilation. Mixed, missing and unverified receipts remain
separate; a later local hit does not prove a fresh-runner restore.
