...
healthchecks pass. Restoration and publication use an atomic hardlink, with a copy
fallback when links are unavailable. The detached payload prevents cache or
runtime corruption from reaching Cargo's retained output in `deps`. A local lock
