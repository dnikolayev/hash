...
def write_payload(payload: Path, binary: Path, key: str) -> tuple[str, dict[str, list]]:
    ...
    return snapshot_hash, installed
...
def _compile_graph_locked(root: Path, original: dict[str, str]) -> int:
    ...
    if not executable(binary):
        ...
        return 1
    try:
        after, _, _, after_context = fingerprint(root, original)
        if after == key and after_context == context:
            save_ready(root, original, key, after_context, binary)
...
def publish(root: Path, original: dict[str, str]) -> bool:
    ...
    with build_lock(root):
        ...
        binary = root / "target/debug/hash-graph"
        try:
            payload_hash, payload_state = write_payload(payload, binary, expected)
            if (ready_state(root, original) is None
                ...
                return False
            linked = link_or_copy_binary(payload / "hash-graph", binary)
            current = path_state(payload / "hash-graph")
            if (path_state(payload / "manifest.json") != payload_state["manifest.json"]
                    or current is None
                    or any(current[index] != payload_state["hash-graph"][index]
                           for index in (0, 1, 2, 4, 5, 6, 7, 9))
                    or (linked and not binary.samefile(payload / "hash-graph"))
                    or (not linked and file_digest(binary) != payload_hash)):
                raise NoReuse("payload-changed-during-publication")
            save_ready(root, original, expected,
                       {name: state[name] for name in ("source", "tools", "state")}, binary)
        except (NoReuse, OSError, ValueError, KeyError, subprocess.SubprocessError) as error:
            shutil.rmtree(payload, ignore_errors=True)
            clear_ready(root)
            binary.unlink(missing_ok=True)
