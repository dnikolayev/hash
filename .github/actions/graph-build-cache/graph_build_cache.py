...
EXTERNAL_ENV = re.compile(r"^(OPENSSL|ZSTD|LIBZ|CMAKE|BINDGEN|LLVM|LIBCLANG|PKG_CONFIG|"
                          r"LIBRARY_PATH|CPATH|C_INCLUDE_PATH|CPLUS_INCLUDE_PATH|DEP_|SCCACHE_(RECACHE|DISABLE)$)")
...
def event(phase: str, status: str, started: float, key: str = "") -> None:
...
def compiler_environment(root: Path, original: dict[str, str]) -> tuple[dict[str, str], str]:
    ...
    for name, path in sorted(tools.items()):
        if not path.is_file():
            raise NoReuse
        version = run([str(path), "-vV" if name == "rustc" else "--version"], root, env)
        identities.append([name, str(path), file_digest(path), digest(version)])
...
def compile_graph(root: Path, original: dict[str, str]) -> int:
    ...
    except (NoReuse, OSError, ValueError, KeyError, subprocess.SubprocessError):
        event("prepare", "unsupported", started)
        return subprocess.run(COMMAND, cwd=root / "apps/hash-graph", env=original).returncode
    ...
    try:
        ...
        event("reject", "unsupported", started, key)
    return 0
